...
export default function ProfilPage() {
  ...
  const handleSave = async () => {
    if (!user || !supabase || isDemoMode) {
      setMessage("Mode local actif : aucune modification n'est enregistree.");
      return;
    }
    ...
    <AppShell badge="Profil" density="comfortable">
      <Card tone="elevated" style={{ maxWidth: 980 }}>
        <SectionHeader
          ...
          description="Retrouvez ici votre identite de compte, votre niveau actuel et les reglages de base de l'espace membre."
          style={{ marginBottom: 0 }}
        />

        {isDemoMode ? <ErrorState tone="info" title="Mode local actif" description={demoReason || "Les modifications ne sont pas persistees dans cet environnement."} style={{ marginTop: 16 }} /> : null}
        ...
        <div className="cp-grid cp-grid-2" style={{ marginTop: 22 }}>
          <Input label="Nom affiche" value={displayName} onChange={(event) => setDisplayName(event.target.value)} placeholder="Votre nom d'usage" />
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginTop: 24 }}>
          <Button onClick={handleSave} disabled={saving}>{saving ? "Enregistrement..." : "Enregistrer les modifications"}</Button>
