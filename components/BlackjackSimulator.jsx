...
// ── PLAYER BOX ──
function BJBox({p,idx,showCards,highlight,hideChips}){
...
export default function BlackjackSimulator(){
  ...
  useEffect(()=>{
    ...
    timerRef.current=setInterval(()=>{
      setL3TimeLeft(t=>{
        if(t<=1){
          setL3Errors(e=>[...e,{num:l3Num,expected:Math.round(l3Num*1.5),given:"⏱"}]);
    ...
    <div style={{minHeight:"100vh",background:"#080808",color:"#F5F0E8",fontFamily:"'DM Sans',sans-serif"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"82vh",gap:20,padding:"0 16px"}}>
        ...
        <div style={{background:"linear-gradient(135deg,rgba(201,168,76,0.08),rgba(201,168,76,0.02))",border:"1px solid rgba(201,168,76,0.2)",borderRadius:8,padding:"10px 24px",textAlign:"center"}}>
          ...
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:18,fontWeight:900,color:"#C9A84C"}}>Major {jackpot.toLocaleString("fr-FR")}€ · Minor {minorJp.toLocaleString("fr-FR")}€</div>
        </div>
        <div>
          ...
          <div style={{display:"flex",gap:8}}>{TABLES.map(t=>(<div key={t.id} onClick={()=>{setTable(t);setScreen("game");if(diff===1)setTimeout(()=>l1Deal(),50);else if(diff===2)setTimeout(()=>deal2(t),50);else setTimeout(()=>startL3(),50);}} style={{padding:"12px 14px",background:"#141414",border:"1px solid rgba(201,168,76,0.1)",borderRadius:6,cursor:"pointer",textAlign:"center",minWidth:120}} onMouseEnter={e=>{e.currentTarget.style.borderColor="rgba(201,168,76,0.35)";}} onMouseLeave={e=>{e.currentTarget.style.borderColor="rgba(201,168,76,0.1)";}}><div style={{fontFamily:"'Playfair Display',serif",fontSize:20,fontWeight:900,color:"#C9A84C"}}>{t.min}€</div><div style={{fontSize:9,color:"#BFB9AD",marginTop:2}}>Max {t.max}€</div></div>))}</div>
        </div>
        <div>
    ...
    <div style={{minHeight:"100vh",background:"#080808",color:"#F5F0E8",fontFamily:"'DM Sans',sans-serif"}}>
      ...
      <div style={{maxWidth:500,margin:"0 auto",padding:"40px 16px",textAlign:"center"}}>
        ...
        <div style={{marginTop:20}}>
          {l1Result==="ok"?(<div style={{fontSize:18,fontWeight:700,color:"#2E7D46"}}>✓</div>):l1Result==="fail"?(<div style={{fontSize:18,fontWeight:700,color:"#C62828"}}>✗</div>):(
  ...
  if(phase==="l3")return(
    <div style={{minHeight:"100vh",background:"#080808",color:"#F5F0E8",fontFamily:"'DM Sans',sans-serif"}}>
      ...
      <div style={{maxWidth:500,margin:"0 auto",padding:"40px 16px",textAlign:"center"}}>
        {!l3Done?(
          <>
            {l3Comp&&<div style={{fontSize:10,fontWeight:700,color:"#C9A84C",marginBottom:6}}>Compétition · {l3Round}/50 · ⏱ {l3TimeLeft}s</div>}
            {!l3Comp&&<div style={{fontSize:10,fontWeight:700,color:"#C9A84C",marginBottom:6}}>Calcule à 1.5 (paiement BJ 3:2)</div>}
            <div style={{background:"radial-gradient(ellipse,#1A6B4F,#0F3D1F)",borderRadius:16,padding:"40px 24px",border:"3px solid rgba(201,168,76,0.15)",marginBottom:20}}>
              <div style={{fontFamily:"'Playfair Display',serif",fontSize:48,fontWeight:900,color:"#F5F0E8"}}>{l3Num}€</div>
              <div style={{fontSize:11,color:"rgba(201,168,76,0.5)",marginTop:4}}>× 1.5 = ?</div>
            </div>
            <div style={{display:"flex",gap:6,justifyContent:"center"}}>
              <input ref={l3Ref} type="number" value={l3Input} onChange={e=>setL3Input(e.target.value)} onKeyDown={e=>e.key==="Enter"&&l3Input.trim()&&submitL3()} placeholder="Résultat" style={{width:160,padding:"10px 14px",background:"#0a0a0a",border:"1px solid rgba(201,168,76,0.2)",borderRadius:4,color:"#F5F0E8",fontSize:20,fontWeight:700,fontFamily:"'DM Sans',sans-serif",outline:"none",textAlign:"center"}} autoFocus/>
          ...
          <div>
            ...
            {l3Errors.length>0&&(
              <div style={{background:"#141414",border:"1px solid rgba(201,168,76,0.08)",borderRadius:6,padding:12,maxHeight:300,overflowY:"auto",textAlign:"left"}}>
                {l3Errors.map((e,i)=>(<div key={i} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:"1px solid rgba(255,255,255,0.03)",fontSize:11,color:"#BFB9AD"}}><span>{e.num}€ × 1.5</span><span style={{color:"#C62828"}}>{e.given} <span style={{color:"#888"}}>→</span> <span style={{color:"#2E7D46"}}>{e.expected}€</span></span></div>))}
              </div>
            )}
      ...
      <div style={{maxWidth:1100,margin:"0 auto",padding:"8px 8px"}}>
        <div style={{background:"radial-gradient(ellipse 120% 80% at 50% 50%,#1A6B4F,#15553A 40%,#0F3D1F 80%,#0B3320)",borderRadius:"12px 12px 50% 50%/12px 12px 18% 18%",padding:"14px 16px 24px",border:"4px solid rgba(201,168,76,0.18)",boxShadow:"inset 0 0 100px rgba(0,0,0,0.2),0 16px 50px rgba(0,0,0,0.45),0 0 0 6px #111",position:"relative",minHeight:400}}>
          ...
          <div style={{display:"flex",justifyContent:"center",gap:4,flexWrap:"wrap",marginBottom:16,position:"relative",zIndex:2}}>
            {players.map((p,i)=><BJBox key={i} p={p} idx={i} showCards={true} highlight={i===activeBox&&step==="play"} hideChips={true}/>)}
          </div>
